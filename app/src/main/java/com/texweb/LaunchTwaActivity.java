...
public class LaunchTwaActivity extends Activity {
    private static final Uri LAUNCH_URI = Uri.parse("https://rest-apis-web.herokuapp.com");

    private final TrustedWebActivityIntentBuilder builder = new TrustedWebActivityIntentBuilder(LAUNCH_URI);
    ...
    public void launchWithCustomReferrer(View view) {
        ...
        if (!serviceBound) {
            CustomTabsClient.bindCustomTabsService(this, action.provider, customTabsServiceConnection);
            serviceBound = true;
        }
    }
